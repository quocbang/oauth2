...
func (l LogLevel) Error(ctx context.Context, msg string, i ...interface{}) {
	if l.level >= logger.Error {
		middleware.GetLoggerFormContext(ctx).Error(fmt.Sprintf(msg, i...), zap.String("caller", utils.FileWithLineNum()))
	}
}
...
func (l LogLevel) Info(ctx context.Context, msg string, i ...interface{}) {
	if l.level >= logger.Info {
		middleware.GetLoggerFormContext(ctx).Info(fmt.Sprintf(msg, i...), zap.String("caller", utils.FileWithLineNum()))
	}
}
...
func (l LogLevel) Warn(ctx context.Context, msg string, i ...interface{}) {
	if l.level >= logger.Warn {
		middleware.GetLoggerFormContext(ctx).Warn(fmt.Sprintf(msg, i...), zap.String("caller", utils.FileWithLineNum()))
	}
}
...
func (l LogLevel) Trace(ctx context.Context, begin time.Time, fc func() (sql string, rowsAffected int64), err error) {
	...
	if err != nil && l.level >= logger.Error {
		...
		middleware.GetLoggerFormContext(ctx).Error("tracing SQL..", fields...)
	} else {
		middleware.GetLoggerFormContext(ctx).Info("tracing SQL", fields...)
	}
}
...
func (rl *RedisLogger) Printf(ctx context.Context, format string, v ...interface{}) {
	middleware.GetLoggerFormContext(ctx).Info("Tracing Redis SQL..", zap.String("sql", fmt.Sprintf(format, v...)))
}
